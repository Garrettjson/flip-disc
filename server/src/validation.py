...
def validate_panel_dimensions(widths: Set[int], heights: Set[int]) -> None:
    ...
    if heights != {7}:
        raise PanelValidationError(
            f"Protocol expects panel height 7, got {sorted(heights)}"
        )

    # All panels must use the same supported width (7, 14, or 28)
    if not (widths <= {7, 14, 28}):
        raise PanelValidationError(
            f"Unsupported panel widths. Expected all 7, 14, or 28; got {sorted(widths)}"
        )
    if len(widths) != 1:
        raise PanelValidationError(
            f"All panels must share the same width; got mixed widths {sorted(widths)}"
        )
...
def validate_canvas_region_bounds(
    x0: int,
    y0: int,
    w: int,
    h: int,
