...
class SerialController:
    ...
    def __init__(self, display_config: DisplayConfig, use_hardware: bool = None):
        ...
        self.logger.info(f"Serial controller initialized with {self.config.panel_count} panels")
        self.logger.info(f"Canvas dimensions: {self.config.canvas_size.w}x{self.config.canvas_size.h}")
    ...
    async def send_canvas_frame(self, canvas_bits: bytes) -> bool:
        ...
        try:
            ...
            # Send to all panels
            success = await self.writer.write_panel_data(panel_arrays)
            
            if success:
                self.logger.debug(f"Canvas frame sent to {len(panel_arrays)} panels")
            else:
                self.logger.error("Failed to send canvas frame to some panels")
    ...
    async def send_panel_frame(self, panel_address: int, frame_data: np.ndarray) -> bool:
        """
        ...
        if frame_data.shape != expected_shape:
            self.logger.error(f"Panel {panel.id} frame shape {frame_data.shape} != expected {expected_shape}")
            return False
        
        success = await self.writer.write_single_panel(panel_address, frame_data)
        
        if success:
            self.logger.debug(f"Frame sent to panel '{panel.id}' (address {panel_address})")
        else:
            self.logger.error(f"Failed to send frame to panel '{panel.id}' (address {panel_address})")
...
# Legacy compatibility - create factory function matching old interface
def create_mock_serial_controller(display_config: DisplayConfig) -> SerialController:
    """
    Create a serial controller with mock writer (for backward compatibility).
    
    Args:
        display_config: Display configuration
        
    Returns:
        SerialController: Controller using mock writer
    """
    logger.info("Creating mock serial controller (legacy interface)")
    return SerialController(display_config, use_hardware=False)