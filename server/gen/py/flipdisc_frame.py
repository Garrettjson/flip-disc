...
class FlipdiscFrame(KaitaiStruct):
    ...
    def _read(self):
        ...
        if not self.magic == b"\x46\x44\x49\x53":
            raise kaitaistruct.ValidationNotEqualError(b"\x46\x44\x49\x53", self.magic, self._io, u"/seq/0")
        self.seq = self._io.read_u2le()
        self.timestamp = self._io.read_u4le()
        self.width = self._io.read_u2le()
        if not self.width >= 1:
            raise kaitaistruct.ValidationLessThanError(1, self.width, self._io, u"/seq/3")
        if not self.width <= 1024:
            raise kaitaistruct.ValidationGreaterThanError(1024, self.width, self._io, u"/seq/3")
        self.height = self._io.read_u2le()
        if not self.height >= 1:
            raise kaitaistruct.ValidationLessThanError(1, self.height, self._io, u"/seq/4")
        if not self.height <= 1024:
            raise kaitaistruct.ValidationGreaterThanError(1024, self.height, self._io, u"/seq/4")
        self.payload_len = self._io.read_u2le()
        _ = self.payload_len
        if not self.payload_len == self.expected_payload_len:
            raise kaitaistruct.ValidationExprError(self.payload_len, self._io, u"/seq/5")
